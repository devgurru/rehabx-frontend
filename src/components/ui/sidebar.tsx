...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => {
    ...
    return typeof window !== 'undefined' ? window.innerWidth >= 1280 : defaultOpen;
  });
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'bg-background relative flex w-full min-w-0 flex-1 flex-col lg:peer-data-[variant=inset]:m-2 lg:peer-data-[variant=inset]:ml-0 lg:peer-data-[variant=inset]:rounded-xl lg:peer-data-[variant=inset]:shadow-sm lg:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2',
        className,
      )}
      {...props}
    />
