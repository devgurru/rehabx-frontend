...
import { CARE_PLAN_STEPS, draftCleared } from '@/store/carePlanSlice';
...
// No static META needed here anymore since we use t('steps.XYZ.title')
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight">
              {t('title', { name: patient.data.fullName })}
            </h1>
            <p className="text-muted-foreground text-sm">
              {t('years', { count: patient.data.age })} · {patient.data.diagnosis.name}
            </p>
          </div>
        </div>
                ...
                <li key={step} className="shrink-0">
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      <span className="block text-sm font-medium">{t(`steps.${step}.title`)}</span>
            ...
            <CardContent className="flex flex-col items-center gap-5 px-6 py-14 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground mx-auto max-w-md text-sm">
                  {t('ready.description', { name: patient.data.firstName })}
                </p>
              </div>
              <div className="bg-brand-soft text-accent-foreground flex items-center gap-2 rounded-full px-4 py-2 text-sm">
                <Smartphone className="size-4" aria-hidden />{' '}
                {t('ready.sharedWith', { name: patient.data.caregiver.name, relationship: patient.data.caregiver.relationship })}
              </div>
          ...
          <Card className="pb-0">
            <CardHeader>
              <p className="text-primary text-xs font-medium">
                {t('step', { current: stepIndex + 1, total: CARE_PLAN_STEPS.length })}
              </p>
              <CardTitle className="text-xl">{t(`steps.${draft.step}.title`)}</CardTitle>
              <CardDescription>{t(`steps.${draft.step}.description`)}</CardDescription>
            </CardHeader>
            <CardContent>
